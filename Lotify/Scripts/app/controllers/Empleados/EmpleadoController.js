app.controller('EmpleadoController', function ($scope, $timeout, $http, appService) {
               ...
               .then(function(results){
                  if(results.data !== ''){
                      ...
                      $scope.resultados = $scope.resultados.filter(function (element) {
                          return element.FechaNacimiento = new Date(parseInt(
                              element.FechaNacimiento.replace("/Date(", "").replace(")/", ""), 10));
                      })
